@extends('layouts.app')
    ...
    <div class="main-panel">
        <div class="content-wrapper">
            ...
            <div class="card">
                <div class="card-body">
                    <h4 class="card-title"> Sub Packages</h4>
                    <div class="row">
                        <div class="col-12">
                        <a href="{{route('create-subpackages',request()->segment(count(request()->segments())))}}" class="btn btn-primary btn-rounded btn-fw float-right" style="margin-top: -50px;" width="250px">Add Sub Packages</a>
                            <div class="table-responsive">
                                <table id="about_dubai_category" class="table dataTable no-footer">
                                    ...
                                    <tbody>
                                    <?php $i=1;?>
                                        @foreach($subpackage as $subpackage_value)
                                            <tr>
                                                <td>{{$i}}</td>
                                                <td>{{$subpackage_value->subpackage_title}}</td>
                                                <td>{{$package}}</td>

                                                <td>
                                                @if($subpackage_value->status == 1)
                                                <label class="switch">
                                                    <input type="checkbox" id="category_status_{{$subpackage_value->id}}" checked onclick="return showSwal('success-message'),change_status({{$subpackage_value->id}})" value="0">
                                                    <span class="slider round"></span>
                                                </label>
                                                @else
                                                <label class="switch">
                                                    <input type="checkbox" id="category_status_{{$subpackage_value->id }}" onclick="return change_status({{$subpackage_value->id}})" value="1">
                                                    <span class="slider round"></span>
                                                </label>
                                                @endif
